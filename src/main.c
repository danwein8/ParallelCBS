...
static void broadcast_instance(ProblemInstance *instance, int root, MPI_Comm comm)
{
    ...
    // Broadcast agent start and goal positions
    if (agents > 0)
    {
...
int main(int argc, char **argv)
{
    ...
    int config_ok = 1;
    // Validate configuration on rank 0
    if (world_rank == 0)
    {
    ...
    // Broadcast configuration status
    ...
    if (world_rank == 0)
    {
        ...
        if (expanders < 0)
        {
            expanders = available > 1 ? available - 1 : available;
        }
        if (low_level_pool < 0)
        {
            low_level_pool = available - expanders;
            if (low_level_pool < 1 && available >= 2)
            {
                low_level_pool = 1;
        ...
        if (expanders + low_level_pool > available)
        {
            low_level_pool = available - expanders;
        }
        if (low_level_pool < 0)
